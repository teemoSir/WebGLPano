...
function PhotoSphereViewer(options) {
  ...
  if (this.config.longitude_range && this.config.longitude_range.length !== 2) {
    ...
    console.warn('warning: longitude_range must have exactly two elements.');
  }
  ...
  if (this.config.latitude_range) {
    ...
    if (this.config.latitude_range.length !== 2) {
      ...
      console.warn('warning: latitude_range must have exactly two elements.');
    }
    // latitude range must be ordered
      ...
      console.warn('warning: latitude_range values must be ordered.');
    }
  }
  // migrate legacy tilt_up_max and tilt_down_max
    ...
    console.warn('warning: tilt_up_max and tilt_down_max are deprecated, use latitude_range instead.');
  }
  ...
  if (this.config.max_fov < this.config.min_fov) {
    ...
    console.warn('warning: max_fov cannot be lower than min_fov.');
  }
  ...
  if (this.config.cache_texture && (!PSVUtils.isInteger(this.config.cache_texture) || this.config.cache_texture < 0)) {
    ...
    console.warn('warning: invalid value for cache_texture');
  }

  // panorama_roll is deprecated
  if ('panorama_roll' in this.config) {
    this.config.sphere_correction.roll = this.config.panorama_roll;
    console.warn('warning: panorama_roll is deprecated, use sphere_correction.roll instead');
  }

  // gyroscope is deprecated
  if ('gyroscope' in this.config) {
    console.warn('warning: gyroscope is deprecated, the control is automatically created if DeviceOrientationControls.js is loaded');
  }
      ...
      if (xhr.status === 200 || xhr.status === 201 || xhr.status === 202 || xhr.status === 0) {
        ...
        if (a === -1 || b === -1 || data.indexOf('GPano:') === -1) {
          ...
          if (!pano_data.full_width || !pano_data.full_height || !pano_data.cropped_width || !pano_data.cropped_height) {
            console.warn('warning: invalid XMP data');
            defer.resolve(null);
          }
          else {
...
PhotoSphereViewer.prototype._loadTexture = function(panorama) {
  ...
  if (Array.isArray(panorama)) {
    ...
    if (this.config.fisheye) {
      console.warn('warning: fisheye effect with cubemap texture can generate distorsions.');
    }
...
PhotoSphereViewer.prototype._transition = function(texture, position) {
  ...
  if (this.prop.isCubemap) {
    if (position) {
      console.warn('warning: cannot perform cubemap transition to different position.');
      position = undefined;
    }
  ...
  if (position) {
    ...
    if (this.config.latitude_range || this.config.longitude_range) {
      ...
      console.warn('warning: trying to perform transition with longitude_range and/or latitude_range, ranges cleared.');
    }
  }
...
PhotoSphereViewer.prototype._bindEvents = function() {
  ...
  if (this.config.mousemove) {
    this.hud.container.style.cursor = 'default';
  ...
  if (PSVUtils.checkTHREE('DeviceOrientationControls')) {
    return PhotoSphereViewer.SYSTEM.deviceOrientationSupported.then(function(supported) {
      if (supported) {
        ...
        console.warn('warning: gyroscope not available');
        return D.rejected();
      }
    }.bind(this));
  }
...
PhotoSphereViewer.prototype.startNoSleep = function() {
  if (!('NoSleep' in window)) {
    console.warn('warning: NoSleep is not available');
    return;
  }
...
function PSVNavBar(psv) {
  ...
  // migration from object
    ...
    console.warn('warning: hashmap form of "navbar" is deprecated, use an array instead.');
      ...
      switch (button) {
        ...
        default:
          if (button.indexOf('spacer') === 0) {
            console.warn('warning: navbar spacers have been removed.');
          }
          else {
...
PSVNavBar.prototype.getNavbarButton = function(id, silent) {
  ...
  if (!button && !silent) {
    console.warn('warning: button "' + id + '" not found in the navbar.');
  }
